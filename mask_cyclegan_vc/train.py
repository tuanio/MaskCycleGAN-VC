...
class MaskCycleGANVCTraining(object):
    ...
    def __init__(self, args):
        ...
        self.validation_dataset = VCDataset(
            datasetA=self.dataset_A,
            datasetB=self.dataset_B,
    ...
    def train(self):
        ...
        for epoch in range(self.start_epoch, self.num_epochs + 1):
            ...
            for i, (real_A, mask_A, real_B, mask_B) in enumerate(
            ...
            if self.logger.epoch % self.epochs_per_plot == 0:
                with torch.no_grad():
                    ...
                    self.logger.visualize_outputs(
                        {
                            "real_A_spec": real_mel_A_fig,
                            "fake_B_spec": fake_mel_B_fig,
                            "real_B_spec": real_mel_B_fig,
                            "fake_A_spec": fake_mel_A_fig,
                        }
                    )
                    ...
                    real_wav_full_A = decode_melspectrogram(
                        self.vocoder,
                        real_mel_full_A[0].detach().cpu(),
                        self.dataset_A_mean,
                        self.dataset_A_std,
                    ).cpu()
                    fake_wav_full_A = decode_melspectrogram(
                        self.vocoder,
                        fake_mel_full_A[0].detach().cpu(),
                        self.dataset_A_mean,
                        self.dataset_A_std,
                    ).cpu()
                    real_wav_full_B = decode_melspectrogram(
                        self.vocoder,
                        real_mel_full_B[0].detach().cpu(),
                        self.dataset_B_mean,
                        self.dataset_B_std,
                    ).cpu()
                    fake_wav_full_B = decode_melspectrogram(
                        self.vocoder,
                        fake_mel_full_B[0].detach().cpu(),
                        self.dataset_B_mean,
                        self.dataset_B_std,
                    ).cpu()
