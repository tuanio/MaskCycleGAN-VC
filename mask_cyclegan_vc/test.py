...
class MaskCycleGANVCTesting(object):
    ...
    def __init__(self, args):
        ...
        self.dataset = VCDataset(datasetA=source_dataset, datasetB=None, valid=True)
    ...
    def test(self):
        for i, sample in enumerate(tqdm(self.test_dataloader)):
            ...
            if self.model_name == "generator_A2B":
                ...
                wav_fake_B = decode_melspectrogram(
                    self.vocoder,
                    fake_B[0].detach().cpu(),
                    self.dataset_B_mean,
                    self.dataset_B_std,
                ...
                wav_real_A = decode_melspectrogram(
                    self.vocoder,
                    real_A[0].detach().cpu(),
                    self.dataset_A_mean,
                    self.dataset_A_std,
                ...
                torchaudio.save(
                    save_path_orig, wav_real_A, sample_rate=self.sample_rate
                )
            else:
                real_B = sample
                ...
                wav_fake_A = decode_melspectrogram(
                    self.vocoder,
                    fake_A[0].detach().cpu(),
                    self.dataset_A_mean,
                    self.dataset_A_std,
                ...
                wav_real_B = decode_melspectrogram(
                    self.vocoder,
                    real_B[0].detach().cpu(),
                    self.dataset_B_mean,
                    self.dataset_B_std,
                ...
                torchaudio.save(
                    save_path_orig, wav_real_B, sample_rate=self.sample_rate
                )
